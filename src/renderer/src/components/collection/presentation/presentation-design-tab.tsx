...
interface PresentationDesignTabProps {
  content: any[]
...
export function PresentationDesignTab({
  content,
  activeSlideIndex,
  previewSettings,
  handlePreviewSettingsChange
}: PresentationDesignTabProps) {
  return (
    <>
        ...
        {previewSettings?.textEffect === 'highlight' && (
          <div className="mt-3">
            ...
            <div className="flex items-center mt-1">
              <Input
                type="color"
                value={previewSettings?.highlightColor?.replace(/[^#\d]/g, '') || '#ffff00'}
                onChange={(e) =>
                  handlePreviewSettingsChange({
                    ...previewSettings,
                    highlightColor: `rgba(${parseInt(e.target.value.slice(1, 3), 16)}, ${parseInt(e.target.value.slice(3, 5), 16)}, ${parseInt(e.target.value.slice(5, 7), 16)}, 0.3)`
                  })
                }
                className="w-9 h-9 p-1 rounded-l-md border-r-0"
              />
      ...
      <div className="relative" style={{ width: '100%', paddingTop: '56.25%' }}>
        <div className="absolute inset-0 bg-black rounded-lg overflow-hidden">
          ...
          {content[activeSlideIndex] && content[activeSlideIndex].type === 'content' && (
            <div
...
export default PresentationDesignTab